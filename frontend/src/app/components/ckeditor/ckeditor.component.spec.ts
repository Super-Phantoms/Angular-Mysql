		...
		describe( `type="${editorType}"`, () => {
			...
			describe( 'on initialization', () => {
				...
				it( 'should have editorUrl pointing to the latest CKEditor 4 version', () => {
					...
					return whenEvent( 'ready', component ).then( () => {
						expect( component.editorUrl ).toEqual( 'https://cdn.ckeditor.com/4.21.0/standard-all/ckeditor.js' );
					} );
				} );
			...
			describe( 'on destroy', () => {
				it ( 'should not have call runOutsideAngular when destroy before DOM loaded', () => {
					spyOn( fixture.ngZone, 'runOutsideAngular' as never);
...
describe( 'CKEditorComponent detached', () => {
	...
	class DetachableCallbackComponent implements AfterViewInit {
		createEditor: Function | undefined;
		...
		@ViewChild( 'container' ) private containerElement: ElementRef | undefined;
		@ViewChild( 'editor' ) private editorElement: ElementRef | undefined;

		ngAfterViewInit(): void {
			if(this.containerElement && this.editorElement)
