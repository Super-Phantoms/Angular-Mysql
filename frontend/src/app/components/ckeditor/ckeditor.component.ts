...
export class CKEditorComponent implements AfterViewInit, OnDestroy, ControlValueAccessor {
	 ...
	 */
	@Input() editorUrl = '/assets/ckeditor 4.21.0/ckeditor.js';
	...
	private createEditor(): void {
		...
		const userInstanceReadyCallback = this.config && this.config['on'] && this.config['on'].instanceReady;
		const defaultConfig: Partial<CKEditor4.Config> = this.editField === 'subject' ? {
			delayIfDetached: true,
			extraPlugins:
			"basicstyles," + 
			"colorbutton,colordialog," +
			"font,wysiwygarea,sourcearea,"+
			"undo,,widgetselection,widget"
		} : {
			delayIfDetached: true,
			extraPlugins:
			"basicstyles,blockquote," + 
			"button,toolbar,clipboard,panelbutton,panel,floatpanel,colorbutton,colordialog," +
			"templates,contextmenu,copyformatting,div,editorplaceholder,resize,elementspath,"+
			"enterkey,entities,exportpdf,popup,filetools,filebrowser,find,floatingspace,listblock,richcombo,"+
			"font,fakeobjects,forms,format,horizontalrule,htmlwriter,iframe,wysiwygarea,image,indent,indentblock,"+
			"indentlist,smiley,justify,menubutton,language,link,list,liststyle,magicline,maximize,newpage,pagebreak,"+
			"pastetext,pastetools,pastefromgdocs,pastefromlibreoffice,pastefromword,preview,print,removeformat,save,"+
			"selectall,showblocks,showborders,sourcearea,specialchar,scayt,stylescombo,tab,table,tabletools,tableselection,"+
			"undo,lineutils,widgetselection,widget,notificationaggregator,uploadwidget,uploadimage,spacingsliders"
			// "easyimage,dialogui,dialog,a11yhelp,about,basicstyles,bidi,blockquote,clipboard," +
			// "colorbutton,colordialog,menu," +
